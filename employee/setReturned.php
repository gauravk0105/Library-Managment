<?php
    ...
    $time = time();
    $n=executeUpdate("update transaction set tstatus='ret',tdateret=$time where tid='$tid' and mid='$mid'");
    if($n==0)
    {
        echo "fail";
    }
    if($n==1)
    {
        ...
        $bid=$row['bid'];
        $nid=$mid.$id;
        $n= executeUpdate("insert into notification values('$nid','$mid','$bid',$time,'unread','ret')");
        echo "success";
    }
